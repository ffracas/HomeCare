            ...
            if (m_services.size() > 1) {
                if (m_services[otherI].m_completed) {
                    ...
                    if (m_sync == Simultaneous) {
                        if (m_services[otherI].m_arrivalTime == m_services[serviceI].m_arrivalTime 
                                && m_services[otherI].m_departureTime == m_services[serviceI].m_departureTime) {
                            return true;        
                        }
                    } else if (m_)
                }         
...
int ValidatioNode::setTime(string t_service, int t_arrivalTime, int t_departureTime) {
    if (m_services[0].m_service == t_service) {
        if (checkService(t_service, t_arrivalTime, t_departureTime, 0, 1)) { return OK; }
    }
    if (m_services.size() > 1 && m_services[1].m_service == t_service) {
        if (checkService(t_service, t_arrivalTime, t_departureTime, 1, 0)) { return OK; }
    }
